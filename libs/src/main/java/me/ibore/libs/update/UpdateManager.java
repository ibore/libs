...
import me.ibore.libs.util.AppUtils;
...
public class UpdateManager {
    ...
    public UpdateManager init(Context context) {
        ...
        mCurrentVerName = AppUtils.getAppVersionName();
        mCurrentVerCode = AppUtils.getAppVersionCode();
        return this;
    }
    ...
    private void compareVerName() {
        ...
        if (!TextUtils.isEmpty(mMinVerName)) {
            int min = AppUtils.compareVersion(mCurrentVerName, mMinVerName);
            if (min < 0) {
                beginUpdate(true);
                if (mListener != null) {
                    mListener.onCheckResult(RESULT_FORCE);
        ...
        // 版本名小于最新,提示更新
        int last = AppUtils.compareVersion(mCurrentVerName, mLastestVerName);
        if (last < 0) {
            beginUpdate(false);
            if (mListener != null) {
                mListener.onCheckResult(RESULT_NOT_LASTEST);
