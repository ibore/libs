...
public class XVideoPlayer extends FrameLayout implements IXVideoPlayer, TextureView.SurfaceTextureListener {
    ...
    private XVideoPlayerController mController;
    ...
    public void setController(XVideoPlayerController controller) {
        mContainer.removeView(mController);
        mController = controller;
        mController.reset();
        mController.setXVideoPlayer(this);
        LayoutParams params = new LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
    ...
    public void start() {
        if (mCurrentState == STATE_IDLE) {
            XVideoPlayerManager.instance().setCurrentXVideoPlayer(this);
            initAudioManager();
            initMediaPlayer();
            initTextureView();
            addTextureView();
        } else {
            Utils.d("XVideoPlayer只有在mCurrentState == STATE_IDLE时才能调用start方法.");
        }
    }
    ...
    public void restart() {
        if (mCurrentState == STATE_PAUSED) {
            ...
            Utils.d("STATE_PLAYING");
        } else if (mCurrentState == STATE_BUFFERING_PAUSED) {
            mMediaPlayer.start();
            mCurrentState = STATE_BUFFERING_PLAYING;
            mController.onPlayStateChanged(mCurrentState);
            Utils.d("STATE_BUFFERING_PLAYING");
        } else if (mCurrentState == STATE_COMPLETED || mCurrentState == STATE_ERROR) {
            mMediaPlayer.reset();
            openMediaPlayer();
        } else {
            Utils.d("NiceVideoPlayer在mCurrentState == " + mCurrentState + "时不能调用restart()方法.");
        }
    }
    ...
    public void pause() {
        if (mCurrentState == STATE_PLAYING) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            Utils.d("STATE_PAUSED");
        }
        if (mCurrentState == STATE_BUFFERING_PLAYING) {
            mMediaPlayer.pause();
            mCurrentState = STATE_BUFFERING_PAUSED;
            mController.onPlayStateChanged(mCurrentState);
            Utils.d("STATE_BUFFERING_PAUSED");
        }
    }
    ...
    private IMediaPlayer.OnPreparedListener mOnPreparedListener
        ...
        public void onPrepared(IMediaPlayer mp) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            Utils.d("onPrepared ——> STATE_PREPARED");
            mp.start();
            // 从上次的保存位置播放
            if (continueFromLastPosition) {
                long savedPlayPosition = Utils.getSavedPlayPosition(mContext, mUrl);
    ...
    private IMediaPlayer.OnVideoSizeChangedListener mOnVideoSizeChangedListener
        ...
        public void onVideoSizeChanged(IMediaPlayer mp, int width, int height, int sar_num, int sar_den) {
            ...
            Utils.d("onVideoSizeChanged ——> width：" + width + "， height：" + height);
        }
    };
    ...
    private IMediaPlayer.OnCompletionListener mOnCompletionListener
        ...
        public void onCompletion(IMediaPlayer mp) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            Utils.d("onCompletion ——> STATE_COMPLETED");
            // 清除屏幕常亮
    ...
    private IMediaPlayer.OnErrorListener mOnErrorListener
        ...
        public boolean onError(IMediaPlayer mp, int what, int extra) {
            ...
            if (what != -38 && what != -2147483648 && extra != -38 && extra != -2147483648) {
                ...
                mController.onPlayStateChanged(mCurrentState);
                Utils.d("onError ——> STATE_ERROR ———— what：" + what + ", extra: " + extra);
            }
            return true;
        }
    };
        ...
        public boolean onInfo(IMediaPlayer mp, int what, int extra) {
            if (what == IMediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START) {
                ...
                if (mTextureView != null) {
                    ...
                    Utils.d("视频旋转角度：" + extra);
                }
            } else if (what == IMediaPlayer.MEDIA_INFO_NOT_SEEKABLE) {
                Utils.d("视频不能seekTo，为直播视频");
            } else {
    ...
    public void release() {
        ...
        if (isPlaying() || isBufferingPlaying() || isBufferingPaused() || isPaused()) {
            Utils.savePlayPosition(mContext, mUrl, getCurrentPosition());
        } else if (isCompleted()) {
            Utils.savePlayPosition(mContext, mUrl, 0);
        }
        // 退出全屏或小窗口
        if (isFullScreen()) {
            exitFullScreen();
