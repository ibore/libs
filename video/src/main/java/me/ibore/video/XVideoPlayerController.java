 ...
 * description:
 * author: Ibore Xie
 * date: 2018/1/22 13:28
 * website: ibore.me
 * </pre>
 */
public abstract class XVideoPlayerController
        extends FrameLayout implements View.OnTouchListener {

    private Context mContext;
    protected IXVideoPlayer mXVideoPlayer;
    ...
    public void setXVideoPlayer(IXVideoPlayer xVideoPlayer) {
        mXVideoPlayer = xVideoPlayer;
    }
     ...
     *                  <ul>
     *                  <li>{@link XVideoPlayer#STATE_IDLE}</li>
     *                  <li>{@link XVideoPlayer#STATE_PREPARING}</li>
     *                  <li>{@link XVideoPlayer#STATE_PREPARED}</li>
     *                  <li>{@link XVideoPlayer#STATE_PLAYING}</li>
     *                  <li>{@link XVideoPlayer#STATE_PAUSED}</li>
     *                  <li>{@link XVideoPlayer#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link XVideoPlayer#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link XVideoPlayer#STATE_ERROR}</li>
     *                  <li>{@link XVideoPlayer#STATE_COMPLETED}</li>
     *                  </ul>
     */
     ...
     *                 <ul>
     *                 <li>{@link XVideoPlayer#MODE_NORMAL}</li>
     *                 <li>{@link XVideoPlayer#MODE_FULL_SCREEN}</li>
     *                 <li>{@link XVideoPlayer#MODE_TINY_WINDOW}</li>
     *                 </ul>
     */
    ...
    protected void startUpdateProgressTimer() {
        ...
        if (mUpdateProgressTimerTask == null) {
            mUpdateProgressTimerTask = new TimerTask() {
                @Override
                public void run() {
                    XVideoPlayerController.this.post(new Runnable() {
                        @Override
                        public void run() {
                            updateProgress();
                        }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        // 只有全屏的时候才能拖动位置、亮度、声音
        if (!mXVideoPlayer.isFullScreen()) {
            return false;
        }
        // 只有在播放、暂停、缓冲的时候能够拖动改变位置、亮度和声音
        if (mXVideoPlayer.isIdle()
                || mXVideoPlayer.isError()
                || mXVideoPlayer.isPreparing()
                || mXVideoPlayer.isPrepared()
                || mXVideoPlayer.isCompleted()) {
            hideChangePosition();
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        mGestureDownPosition = mXVideoPlayer.getCurrentPosition();
                        ...
                        if (mDownX < getWidth() * 0.5f) {
                            ...
                            mGestureDownVolume = mXVideoPlayer.getVolume();
                        }
                    }
                }
                if (mNeedChangePosition) {
                    long duration = mXVideoPlayer.getDuration();
            ...
            case MotionEvent.ACTION_UP:
                if (mNeedChangePosition) {
                    mXVideoPlayer.seekTo(mNewPosition);
                    hideChangePosition();
